...
function normalizeTool(raw: any): Tool | null {
  try {
    ...
    const toolData = {
      ...
      logo_url: raw.logo_url ? sanitizeUrl(raw.logo_url) : '',
      website_url: raw.website_url ? sanitizeUrl(raw.website_url) : '#',
      rating: raw.rating || parseFloat((4.0 + Math.random() * 1.0).toFixed(1)),
