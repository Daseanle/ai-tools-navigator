...
INSERT INTO users (id, name, email, avatar, provider) VALUES
  ('user-123', '测试用户', 'test@example.com', '/avatars/default.png', 'email'),
  ('user-456', '管理员', 'admin@example.com', '/avatars/admin.png', 'email')
ON CONFLICT (email) DO NOTHING;

-- 插入示例评分数据
INSERT INTO tool_ratings (tool_id, user_id, rating, comment) VALUES
  ((SELECT id FROM tools WHERE slug = 'chatgpt'), 'user-123', 5, '非常好用的AI工具，功能强大！'),
  ((SELECT id FROM tools WHERE slug = 'claude'), 'user-123', 4, '分析能力很强，回答很有深度'),
  ((SELECT id FROM tools WHERE slug = 'midjourney'), 'user-456', 5, '生成的图像质量很高，艺术感很强')
ON CONFLICT (tool_id, user_id) DO NOTHING;

-- 插入示例收藏数据
INSERT INTO user_favorites (user_id, tool_id) VALUES
  ('user-123', (SELECT id FROM tools WHERE slug = 'chatgpt')),
  ('user-123', (SELECT id FROM tools WHERE slug = 'claude')),
  ('user-456', (SELECT id FROM tools WHERE slug = 'midjourney'))
ON CONFLICT (user_id, tool_id) DO NOTHING;

-- 插入示例会员数据
INSERT INTO user_memberships (user_id, plan_type, status, expires_at, amount, currency) VALUES
  ('user-456', 'pro', 'active', NOW() + INTERVAL '30 days', 99.00, 'CNY')
ON CONFLICT DO NOTHING;

-- 插入示例用户设置数据
INSERT INTO user_settings (user_id, theme, language, notifications, privacy) VALUES
  ('user-123', 'dark', 'zh-CN', '{"email": true, "push": false}', '{"profile_public": true, "activity_public": false}'),
  ('user-456', 'light', 'zh-CN', '{"email": true, "push": true}', '{"profile_public": true, "activity_public": true}')
