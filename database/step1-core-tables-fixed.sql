...
-- DROP TABLE IF EXISTS tool_ratings CASCADE;
...
DO $$
DECLARE 
    chat_category_id UUID;
    image_category_id UUID;
    productivity_category_id UUID;
    design_category_id UUID;
    programming_category_id UUID;
BEGIN
    -- 获取分类ID
    SELECT id INTO chat_category_id FROM categories WHERE slug = 'ai-chat';
    SELECT id INTO image_category_id FROM categories WHERE slug = 'image-generation';
    SELECT id INTO productivity_category_id FROM categories WHERE slug = 'productivity';
    SELECT id INTO design_category_id FROM categories WHERE slug = 'design';
    SELECT id INTO programming_category_id FROM categories WHERE slug = 'programming';
    ...
    INSERT INTO tools (name, slug, description, website, category_id, pricing, features, tags, rating, visits, featured, logo, api_available, free_tier, trial_available) VALUES
      ...
      ('Notion AI', 'notion-ai', '集成在Notion中的AI助手，提升工作效率', 'https://notion.so', 
       productivity_category_id, 'Freemium', 
...
BEGIN
    ...
    SELECT id INTO claude_id FROM tools WHERE slug = 'claude';
    SELECT id INTO midjourney_id FROM tools WHERE slug = 'midjourney';
    
    -- 插入评分数据
    INSERT INTO tool_ratings (tool_id, user_id, rating, comment) VALUES
      (chatgpt_id, '11111111-1111-1111-1111-111111111111', 5, '非常好用的AI工具，功能强大！'),
      (claude_id, '11111111-1111-1111-1111-111111111111', 4, '分析能力很强，回答很有深度'),
      (midjourney_id, '22222222-2222-2222-2222-222222222222', 5, '生成的图像质量很高，艺术感很强')
    ON CONFLICT DO NOTHING;
    
    -- 插入收藏数据
    INSERT INTO user_favorites (user_id, tool_id) VALUES
      ('11111111-1111-1111-1111-111111111111', chatgpt_id),
      ('11111111-1111-1111-1111-111111111111', claude_id),
      ('22222222-2222-2222-2222-222222222222', midjourney_id)
    ON CONFLICT DO NOTHING;
END $$;
