...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Wechat payment callback:', callbackData)
    
    const result = await PaymentManager.handlePaymentCallback('wechat_pay', callbackData)
    ...
    return new NextResponse('<xml><return_code><![CDATA[FAIL]]></return_code><return_msg><![CDATA[ERROR]]></return_msg></xml>', {
      headers: { 'Content-Type': 'application/xml' }
    })
  }
}
